...
class VideoVisualizer:
    ...
    def draw_instance_predictions(self, frame, predictions):
        ...
        periods = predictions.ID_period if predictions.has(
            "ID_period") else None
    ...
    def draw_panoptic_seg_predictions(
        ...
        masks_rles = mask_util.encode(
            np.asarray(np.asarray(masks).transpose(
                1, 2, 0), dtype=np.uint8, order="F")
        )
        assert len(masks_rles) == num_instances
    ...
    def _assign_colors(self, instances):
        ...
        if instances[0].bbox is None:
            ...
            threshold = 0.6
        if len(ious) == 0:
            ious = np.zeros((len(self._old_instances),
                            len(instances)), dtype="float32")
